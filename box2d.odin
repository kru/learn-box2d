...
draw_entity :: proc(e: Entity) {
	...
	rec := rl.Rectangle{
		p.x, p.y,
		e.extent.x*2, e.extent.y*2
	}
	rotation := rl.RAD2DEG * b2.Rot_GetAngle(b2.Body_GetRotation(e.body_id))
	rl.DrawTexturePro(e.texture, e.texture_rect,
		rec, e.extent, rotation, {255,255,255,255}
	)
}
...
create_box :: proc(world_id: b2.WorldId, position: b2.Vec2, rotation: f32) -> Entity {
	...
	box.shape_id = b2.CreatePolygonShape(
		box.body_id,
		box_shape_def,
		box_polygon
	)
...
create_ground :: proc(world_id: b2.WorldId, position: b2.Vec2) -> Entity {
	...
	ground_shape_def.material.friction = 5.0
	ground.shape_id = b2.CreatePolygonShape(
		ground.body_id, ground_shape_def, ground_polygon
	)
...
main :: proc() {
	rl.InitWindow(1920, 1080, "Box2D Idea")
	...
	for i in 0..<100 {
		...
		append(&entities,
			create_box(world_id, {
				600 + 100*math.mod(x, 8) + 50*math.sin(x), 100+200*x/8
			}, x)
		)
	}
